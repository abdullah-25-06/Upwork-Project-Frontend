...
function Addcar() {
  ...
  useEffect(() => {
    setMake(id && ctx.selectedCar?.make);
    setModel(id && ctx.selectedCar?.model);
    setYear(id && ctx.selectedCar?.year);
    setVehicle(id && ctx.selectedCar?.vehicleNo);
    setChasis(id && ctx.selectedCar?.chasisNo);
    setSelectedValue(id && ctx.selectedCar?.FuelType);
    setImages(id && [ctx?.selectedCar?.images[0]]);
  }, [id, ctx]);
